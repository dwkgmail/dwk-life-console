...
const dataCode=await readFile(new URL('../js/data.js',import.meta.url),'utf8');
const utilsCode=await readFile(new URL('../js/utils.js',import.meta.url),'utf8');
const storageCode=await readFile(new URL('../js/storage.js',import.meta.url),'utf8');
...
test('PWA、Markdown 导出与移动端防横向滚动配置存在',async()=>{
  const [manifest,sw,app,css]=await Promise.all(['../manifest.json','../sw.js','../js/app.js','../css/style.css'].map(x=>readFile(new URL(x,import.meta.url),'utf8')));const m=JSON.parse(manifest);assert.equal(m.display,'standalone');assert.match(sw,/skipWaiting/);assert.match(sw,/caches\.delete/);assert.match(app,/text\/markdown/);assert.match(css,/overflow-x:hidden/);
});
